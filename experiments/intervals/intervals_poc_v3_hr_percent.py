...
WORKOUT_NAME = "GWG PoC 3 - HR percent"
EXTERNAL_ID_PREFIX = "garmin-workout-generator-poc3"

def build_workout_description() -> str:
    return """Warmup
- 5m 70-80% HR intensity=warmup

2x
- 1m 85-90% HR intensity=interval
- 2m 70-80% HR intensity=recovery

Cooldown
- 5m 70-80% HR intensity=cooldown"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="PoC 3: target FC percentuale via Intervals.icu -> Garmin."
    )
    ...
    print("Workout PoC 3 creato/aggiornato con successo.")
    for event in events:
        print(json.dumps({
            "id": event.get("id"),
            "name": event.get("name"),
